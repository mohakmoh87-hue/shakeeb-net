...
export default function InstallApp() {
  ...
  const [modal, setModal] = useState<"ios" | "qr" | "hint" | null>(null);

  useEffect(() => {
    const standalone = window.matchMedia("(display-mode: standalone)").matches || (navigator as unknown as { standalone?: boolean }).standalone === true;
    if (standalone) { setPlat("installed"); return; }
  ...
  const label = plat === "ios" ? "📲 تثبيت على آيفون" : plat === "desktop" ? "🖥️ فتح على الهاتف" : "📲 تثبيت التطبيق";
    ...
    <>
      <div className="mt-4 flex items-center gap-3 rounded-2xl border border-mynet-blue/20 bg-white/70 p-3 shadow-sm">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[11px] text-slate-500">ثبّته على هاتفك ليعمل كتطبيق مستقلّ بإشعارات فورية</div>
        </div>
