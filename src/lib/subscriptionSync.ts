...
async function runOfficeSyncInner(
  ...
  for (const a of acts) {
    ...
    // ═══ 📨 (أُزيل من هنا) · مُطلِقُ رسالةِ «فعّل بنفسه» — بلاغُ محمد 2026-08-25 ═══
    // 🔴 **كان هنا شرطٌ يتيمٌ لا يعرف السجلَّ**: `autoMsg.self && !managerMatch` — أي
    //   «المنجرُ ليس حسابَ المكتب ⇒ أرسِلْ». وقِيس أثرُه على الإنتاج ليلتَها: **٤٨ رسالةً
    //   خاطئةً من ١١٥** في ثلاثين ساعة (٢٢ لمن لا صفَّ له في السجلّ أصلاً · ٢٦ لأصحاب
    //   قروض)، وستّةٌ وصلهم **قالبٌ ليس قالبَهم**.
    //   والسببُ أنّ التبويبَ يُبنى بأربعة حرّاسَ لا يمرّ عليها هذا الشرطُ إطلاقاً: «مغطّى»
    //   و«مقبوضٌ عندي» يُسقطان الواقعةَ من السجلّ، و«القرض» يختمها، والتصنيفُ الثلاثيُّ
    //   يفرّق كابينةَ صاحبه عن الديلر/الشركة.
    // ⇒ **الرسالةُ صارت تتبع الحدثَ لا المنجرَ وحدَه**: تُطلَق من حلقة التصنيف أدناه
    //   لحظةَ **ولادة صفٍّ جديدٍ فعلاً** (`outcome === "created"`) — تماماً كما يفعل مسارُ
    //   «تنصيب خارجي» للجدد (`if (fresh && autoMsg.install)`). فما لا صفَّ له لا رسالةَ له.
  ...
  for (const a of actsWide) {
    ...
    // ═════ 📨 الرسالةُ تتبع **الحدثَ** لا الشخص (قاعدةُ محمد 2026-08-25) ═════
    // بنصّه: «لكلّ حالةٍ وحسب الحالة تصل الرسالة» — فالمشتركُ الواحد قد تصله ثلاثُ رسائلَ
    // مختلفةٍ في ثلاثة أشهرٍ بحسب كيف فُعِّل كلَّ مرّة:
    //   · فعّل بنفسه (منجرُه كابينتُه) ⇒ قالب **«تفعيل خارجي»**.
    //   · ديلر/شركة ⇒ قالب **«تنصيب خارجي»** — «ولا أحدَ غيرهم تصله رسالةُ هذا القالب».
    //   · منجرٌ = صفحةُ المكتب (تبويب «تفعيلات ساس») ⇒ **لا رسالةَ من المزامنة**: هذه
    //     واقعةٌ تنتظر تسجيلَ وصلها، وحين يُسجَّل تخرج **رسالةُ التفعيل العاديّة** من
    //     البرنامج نفسِه — فرسالةٌ هنا تعني رسالتَين لتفعيلٍ واحد.
    //   · قرضٌ (مبلغُ صفر) ⇒ **لا رسالةَ**: ليس تفعيلاً اشتراه، وهو أحدُ وجهَي بلاغ محمد.
    // 🔑 **والشرطُ `created` هو الحارسُ كلُّه**: لا رسالةَ إلّا لصفٍّ وُلد الآن فعلاً. فما
    //    أسقطه حارسُ «مغطّى» أو «مقبوضٌ عندي» أعلاه لم يصل إلى هنا أصلاً (`continue`)،
    //    وإعادةُ قراءةِ تفعيلةِ الأمس كلَّ دورةٍ تُعيد `open`/`closed` لا `created`.
    //    وفوقه حارسُ `dedupKey` الفيزيائيُّ في `sendSyncLogMessage` — طبقتان لا واحدة.
    if (outcome === "created" && !isLoanAct) {
      const msgKind = managerIsPage ? null : ownCabinet ? "self" as const : "install" as const;
      if (msgKind && (msgKind === "self" ? autoMsg.self : autoMsg.install)) {
        await sendSyncLogMessage(msgKind, {
          towerId: officeId, sasId: a.sasUserId, activatedAt: actAt,
          subscriberId: sub.id, netUser: evBase.netUser, name: evBase.name,
          sasDateTo: validNewExp,
        });
      }
    }
