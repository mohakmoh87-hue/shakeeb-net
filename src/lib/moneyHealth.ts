...
export async function runMoneyHealth(agentId: number): Promise<{ checks: HealthCheck[]; summary: Row }> {
  ...
  // يُغلَق وحدَه فور ظهور الوصل (يقلبه المحرّكُ إلى internal)، ويمكن تجاهلُه كبقيّة الحالات.
  await add("contract_theft", "كلُّ عقدٍ في موقع العقود له وصلٌ في البرنامج", `
    ...
    how: "المكتبُ أنشأ عقدَ تنصيبٍ في موقع العقود، لكن لا وصلَ في البرنامج باسم المستخدم نفسِه — قد يكون المالُ لم يُسجَّل. ابحث عن وصله باليوزر وسجّله؛ يُغلَق هذا الإنذارُ وحدَه فور ظهوره، أو تجاهَله إن كان تعويضاً/اختباراً.",
  ...
  await add("card_used_zero_price", "كلُّ كارتٍ مستخدَمٍ له سعر", `
    ...
    how: "اضبط سعرَ هذه الفئة من «أسعار الكروت» في هذه الصفحة — فالكروتُ الجديدةُ تأخذه تلقائيّاً. وأمّا القائمةُ فسعرُها يُصحَّح من صفحة الكروت (تحديدٌ جماعيٌّ ثمّ تعديلُ السعر).",
    severity: "critical", at: s(x.at),
  }));
  ...
  // ── و-٣ · 🎴 تفعيلاتٌ بكروتٍ **من خارج المخزن** (بيتُها هنا — إملاءُ محمد 2026-08-21) ──
  //
  // حالةُ «جاسم محمد طلال علوان عيساوي»: فُعِّل بثلاثة كروتٍ كلُّها خارج المخزن، فحسبته
  // المزامنةُ «تفعيلاً خارجيّاً ومكرَّراً» ثمّ **لم تُخزّنه في أيّ مكان** — ذُكر في تقرير
  // مزامنةٍ عابرٍ وضاع. ونصُّ محمد: «أيُّ حالةٍ في المزامنة ليس لها مكانٌ فيجب أن تكون في
  // حارس المال، وتُدقَّق من كلّ النواحي — هل له وصل؟ بكم؟ هل هو ديلر؟ — مع ذكر التقييم».
  //
  // والتحليلُ يجري في المزامنة (حيث بياناتُ الساس حاضرة) ويُخزَّن جاهزاً، وهنا يُعرَض:
  //   🤝 ديلر (كارتان فأكثر + وصلٌ في نافذة ٣٦ ساعة) ⇒ **بسيطةٌ غيرُ ضارّة** — وقاعدةُ محمد
  //      أنّ ديلرَ الثلاثة أشهرٍ يُفعَّل بكروتٍ خارجيّةٍ دائماً ووصلُه ٣ أشهرٍ ناقص ٥ آلاف.
  //   🎴 وصلٌ موجودٌ بكارتٍ واحدٍ خارجيّ ⇒ لا ضرر (كارتٌ لم يُسجَّل في المخزن).
  //   🔴 بلا وصلٍ إطلاقاً ⇒ مالٌ لم يُقبَض عندك — يحتاج قراراً.
  await add("ext_card_activations", "لا تفعيلَ بكارتٍ خارجَ المخزن بلا تفسير", `
    SELECT g.id, g."netUser", g.name, g.amount, g.note, g.changes,
    ...
    return {
      rowKey: `extcard:${s(x.id)}`,
      // 🔴 **«مسروق» حكمٌ مستقلٌّ لا يُخلَط بالخارجيّ** (إملاءُ محمد 2026-08-22): الخارجيُّ
      //    كارتٌ **ليس من مخزننا** (مخزنُنا لم يُمَسّ)، والمسروقُ كارتٌ **من مخزننا** خرج
      //    وفُعِّل بلا وصلٍ لصاحبه ⇒ مالٌ ذهب ولم يدخل.
      title: verdict === "stolen"
        ? "🔴 كارتٌ من مخزنك فُعِّل بلا وصل — يُشتبه بسرقة"
        : verdict === "dealer"
          ? "تفعيلُ ديلرٍ بكروتٍ من خارج المخزن (طبيعيّ)"
          : verdict === "receipted"
            ? "تفعيلٌ بكارتٍ من خارج المخزن — وله وصلٌ عندك"
            : "تفعيلٌ بكارتٍ من خارج المخزن بلا وصل",
      detail: `${s(x.name) || s(x.netUser) || "—"} · ${s(x.at)}${x.amount ? ` · بمبلغ ${n(x.amount)}` : ""}
${s(x.note)}`,
      how: verdict === "stolen"
        ? "الكارتُ عُلّم مستخدماً تلقائيّاً كي لا يُسحَب ويُباع ثانيةً، ولم يتحرّك أيُّ مال. تحقّق: هل قبض أحدٌ ثمنَه؟ إن كان مدفوعاً فسجّل وصلَه — وتُغلَق الحالةُ وحدَها في المزامنة التالية. وإن لم يكن فاسأل مَن سحبه (اسمُه في التفصيل)."
        : verdict === "no-receipt"
          ? "افتح المشتركَ وتحقّق: إن كان قد دفع فسجّل وصلَه، وإن فعّله غيرُك (ديلر/شركة) فاطلب تفسيرَه. ثمّ «تجاهل» ليختفي."
          : "لا إجراءَ مطلوب — الحالةُ مفهومةٌ ومالُها مقبوض. «تجاهل» يُخفيها نهائيّاً.",
      severity: verdict === "stolen" || verdict === "no-receipt" ? "critical" : "info",
      amount: n(x.amount), at: s(x.at),
    };
  });
  ...
  //   والزيادةُ **لا تُبلَّغ أبداً** — و**السببُ صار معروفاً بجوابِ محمد 2026-08-14**:
  //   «سؤالُ الألف أهمِله ولا أريد طرحَه مجدَّداً، فهو **مجرّدُ مبلغِ توصيل**».
  //   فالـ٤٢٧ حالةً بفارقِ +١٠٠٠ **إيرادُ توصيلٍ مشروع**، لا شذوذ. ⛔ ولا يُبنى فحصٌ
  //   لاتّجاه الزيادة بعد اليوم — القرارُ مُغلَق.
