...
export function decideStampOffice(
  ...
  // فيظنّ العطلَ في جهازه ويُعيد المحاولةَ عبثاً. فتُذكَر له العلّةُ الحقيقيّةُ وحلُّها.
  const n = measured[0];
  const unlocated = offices.filter((o) => o.lat == null || o.lng == null || !o.geoEnabled);
  const hint = unlocated.length
    ? ` — وإن كنتَ في ${unlocated.map((o) => `«${o.name ?? "مكتب"}»`).join(" أو ")} فموقعُه غيرُ محدَّدٍ في النظام، فأبلِغ المدير ليُحدّده`
    : "";
  return {
    office: null,
    error: `يجب أن تكون داخل «${n.o.name ?? "المكتب"}» للبصمة — تبعد عنه ~${n.dist} م (المسموح ${n.radius} م)${hint}`,
  };
}
