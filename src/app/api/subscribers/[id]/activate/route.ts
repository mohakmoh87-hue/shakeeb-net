  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.subscriber.update({
        ...
        data: {
          // 🔴 حرِجة ٣ · دَينُ المشترك ذرّيّاً (اصطاده الفحصُ العدائيّ 2026-08-19):
          //   كان يُكتَب `carry: newCarry` قيمةً **مطلقةً** محسوبةً من قراءةٍ قبل المعاملة.
          //   فتسديدُ دينٍ متزامنٌ مع هذا التفعيل يقرأ نفسَ القديم، وآخرُ كتابةٍ تمحو الأولى
          //   ⇒ القبضُ في الصندوق والدَّينُ يرجع كأنّ التسديد لم يكن. والفارقُ (grandTotal-paid)
          //   مستقلٌّ عن القديم فيُطبَّق **إضافةً ذرّيّة**، فتُجمَع مع أيّ تغييرٍ متزامنٍ لا تمحوه.
          //   واللقطةُ `newCarry` تبقى للوصل والتدقيق (تعكس مساهمةَ هذا التفعيل).
          packageId, dateTo, carry: { increment: fullPaid ? 0 : grandTotal - paid }, wasel: effPaid, month: months,
