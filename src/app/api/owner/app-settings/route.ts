...
import { getAppContent, setAppContent, getTicketDest, setTicketDest } from "@/lib/appConfig";
...
export async function GET() {
  ...
  const [content, otpWa, ticketDest] = await Promise.all([getAppContent(), getOtpWaInfo(), getTicketDest()]);
  return NextResponse.json({ ...content, otpWa, ticketDest });
}
