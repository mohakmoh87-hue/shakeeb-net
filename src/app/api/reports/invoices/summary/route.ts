...
// مجاميع يومية للشهر الحالي مقابل نفس المدة من الشهر السابق.
...
export async function GET() {
  ...
  for (const r of rows) {
    ...
    if (r.date >= thisStart) {
      if (day <= today) { thisDays[day - 1] += v; thisTotal += v; thisCount++; }
    } else if (r.date <= lastEnd && day <= today) {
      lastDays[day - 1] += v; lastTotal += v;
    }
  }

  return NextResponse.json({ days: today, thisDays, lastDays, thisTotal, lastTotal, thisCount });
}
