  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 p-6">
      ...
      <p className="text-xs leading-5 text-slate-500">
        فعّل وكيلاً موزّعاً، وأنشئ له حسابَ دخولٍ منفصلاً لصفحة <b dir="ltr">/cadrdist</b>، وحدّد الوكلاءَ الذين يضع في مخازنهم كروتاً. الموزّعُ يغيّر كلمةَ مروره بنفسه لاحقاً.
      </p>
      ...
      {data.distributors.map((d) => (
        <div key={d.agentId} className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="mb-3 text-xs text-slate-500">حساب /cadrdist: <b dir="ltr">{d.username ?? "—"}</b> {d.plainPassword && <span dir="ltr" className="text-slate-400">/ {d.plainPassword}</span>}</div>
