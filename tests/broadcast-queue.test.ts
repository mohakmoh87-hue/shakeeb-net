...
describe("ب-٢ · طابور البثّ الجماعيّ", () => {
  ...
  test("🚨 حادثةُ «أين ذهبت المهلة؟»: طابورُ الحاسبة لا يخطف طابورَ البثّ ولا يُرسل رشقةً", () => {
    ...
    assert.ok(/SELF_ACT_GAP_MS/.test(s), "يُرسل بلا فاصل — رشقةٌ تُعرّض الرقمَ للحظر");
    assert.ok(/setTimeout\(r, SELF_ACT_GAP_MS\)/.test(s), "الفاصلُ معرَّفٌ ولا يُنتظَر فعلاً");
