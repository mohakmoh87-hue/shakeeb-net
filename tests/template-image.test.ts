...
describe("البند ٣ · صورةُ القالب", () => {
  ...
  test("واتساب: صورةٌ ونصٌّ رسالةٌ واحدة، وفشلُ الصورة لا يُسقط النصّ", () => {
    ...
    assert.match(block.slice(0, 1400), /catch[\s\S]*client\.sendMessage\(waId, text\)/, "فشلُ الصورة يُسقط الرسالةَ كلَّها");
    // ═══ وسببُ السقوط يعود مع النتيجة فيُوثَّق في سجلّ الرسائل (بلاغ محمد 2026-08-14) ═══
    // كان محبوساً في `console` على حاسبة المكتب، فتعذّر تشخيصُ «وصلت بلا صورة» عن بُعد.
    assert.match(wa, /imageError/, "سببُ سقوط الصورة لا يُعاد — يبقى حبيسَ نافذة الحاسبة");
    assert.match(wa, /صيغةُ الصورة غير صالحة/, "عدمُ مطابقة data URI يسقط صامتاً بلا سبب");
    assert.match(wa, /rr\.imageError/, "المُرحِّلُ لا يُعيد سببَ الصورة إلى الموقع");
