...
describe("الكروتُ الوهميّة: لا حكمَ بغيابِ دليلٍ لم يثبت أنّه يحضر", () => {
  ...
  test("🎯 لا يُوسَم كارتٌ إلّا بعد فشلِ البحثِ الموجَّه بسيريالِه", () => {
    ...
    assert.match(gate, /sasSearchActivation\(s\.base, s\.token/, "الجردُ لا يتحقّق بالبحث الموجَّه");
    ...
    // وما وُجد يُحسَب مُثبَتاً — فلا يبقى مشتبَهاً به
    assert.match(gate, /if \(foundReal\[i\]\) res\.verifiedReal\+\+/, "ما وجده البحثُ لا يُحسَب حقيقيّاً");
    // وسقفٌ يمنع حلقةً ضخمةً على الساس، والفائضُ **يبقى مشتبَهاً لا يُبرَّأ** ضمناً
    assert.match(gate, /MAX_VERIFY/, "لا سقفَ لعدد عمليّات البحث");
    assert.match(gate, /const stillSuspect: typeof candidates = \[\.\.\.overflow\]/, "الفائضُ فوق السقف يُبرَّأ بلا تحقّق");
