...
describe("📋 سجلّ المزامنة الموحَّد", () => {
  ...
  test("🪟 النافذةُ منتصفَ الشاشة 80% ولا تُغلق إلّا بـ✕ — شرطُ محمد الثابت", () => {
    ...
    // 📱 و`dvh` لا `vh` (2026-08-21): شريطُ متصفّح الهاتف المتحرّك يجعل vh أطولَ من المرئيّ
    //    فيُقصّ أسفلُ النافذة في التطبيق — وهي علّةُ «مقصوصةٌ من أعلى وأسفل» بعينها.
    assert.match(modal, /h-\[92dvh\] w-\[96vw\]/, "الهاتفُ بلا مقاسٍ شبهِ كامل — التبويباتُ تُقصّ");
  ...
  test("📨 طابورُ الرسائل عمرُه ٢٤ ساعة — وحارسُ تكرارٍ فيزيائيّ (نصّ محمد 2026-08-21)", () => {
    const q = read("src/lib/syncAutoMsg.ts");
    // 🔄 **تصحيحُ محمد 2026-08-21 مساءً**: «كلُّ طابورٍ لا يُمسَح أبداً اجعله يُمسَح كلَّ
    //    ٢٤ ساعة» — فالدوامُ صار عمراً محدوداً: رسالةٌ تصل بعد يومَين تُربك المشترك.
    //    والباقي كما هو: التعذُّرُ يُعيد للطابور ولا يختم فشلاً، والحارسُ الفيزيائيُّ قائم.
    assert.ok(q.includes("const QUEUE_MAX_AGE_MS = 24 * 3600_000;"), "الطابورُ بلا عمرٍ محدَّد");
    assert.ok(q.includes('dedupKey: { startsWith: "synclog:" },'), "الحذفُ غيرُ مقيَّدٍ بصفوف هذا الطابور");
    assert.equal(/status: "FAILED"/.test(q), false, "الطابورُ يختم فشلاً — والتعذُّرُ يُعيد للطابور");
