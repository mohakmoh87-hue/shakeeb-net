...
describe("أودو على لوحتَين — الإشعالُ والإطفاءُ لكلّ لوحةٍ على حدة", () => {
  test("مفتاحُ أودو للّوحة قائمٌ — في بيته الوحيد بعد إزالة الازدواج", () => {
    // ═════ إزالةُ الازدواج (قرارُ محمد 2026-08-19) ═════
    // كان لأودو ثلاثةُ مداخل (نموذجُ المكتب · نافذةُ اللوحات · إدارة الفنيّين)، فحُصر
    // في «إعدادات أودو» بصفحة إدارة الفنيّين — قال محمد: «وهو المكان الصحيح الأساسي».
    // والقدرةُ المحروسةُ لم تتغيّر: **إشعالُ أودو للّوحة الثانية ممكنٌ** — وإلّا بقيت
    // "0" فلا تُزامَن أبداً بلا رسالة (العلّةُ الأصليّة التي حرسها هذا الاختبار).
    const cfg = read("src/components/OdooConfigButton.tsx");
    assert.ok(cfg.includes("panelId = null"), "زرُّ الإعدادات يجب أن يقبل لوحةً بعينها (panelId)");
    assert.ok(cfg.includes('odooEnabled: enabledForm ? "1" : "0"'), "لا مفتاحَ إشعالٍ للّوحة ⇒ الثانيةُ لا تُزامَن أبداً");
    // ونافذةُ اللوحات ما عادت تُدخِل أودو — شارةُ حالةٍ فقط، فلا يعود لأودو مدخلان
    const panels = read("src/components/SasPanelsButton.tsx");
    assert.ok(!panels.replace(/hasOdooPass/g, "").includes("odooPass"),
      "عادت حقولُ أودو إلى نافذة اللوحات — وهذا الازدواجُ بعينه الذي أُزيل");
    assert.ok(!panels.includes("toggleOdoo"), "عاد مفتاحُ أودو إلى نافذة اللوحات");
  });
