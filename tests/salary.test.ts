...
// ═════════════════════════════════════════════════
// ب-٠٠ · المبلغُ السالب — **سبعةٌ من ثمانيةٍ صارت مبنيّةً، فتُحرَس** (2026-08-14)
//
// 🔴 وسببُ كتابةِ هذه الحرّاس اليوم: مراجعةٌ للحالة كشفت أنّ **الإصلاحاتَ موجودةٌ في
//   الكود ومواصفاتُها ما زالت `todo`** — أي سبعُ إصلاحاتٍ ماليّةٍ **بلا حارسٍ واحد**.
//   وتعديلٌ غداً يمحو أحدَها بصمتٍ ولا يُفشل اختباراً. فالمواصفةُ التي لا تُنفَّذ
//   حرّاساً هي **وثيقةٌ لا حماية**.
//
// ⚠️ وهذه حرّاسٌ بنيويّةٌ (تقرأ المسارَ نفسَه) لا وحدويّة: هذه مساراتُ HTTP تلمس القاعدة،
//   واختبارُها سلوكيّاً يحتاج قاعدةَ اختبارٍ — وهي غيرُ متاحةٍ بصفرِ تكلفة. والبنيويُّ
//   يُثبّت **القرار** (أيُقصُّ الزائدُ أم يصير رصيداً؟) وهو موضعُ الخطأ الذي وقع فعلاً.
describe("ب-٠٠ · حرّاسُ المبلغ السالب (سبعةٌ مبنيّة)", () => {
  const read = (rel: string) => fs.readFileSync(path.join(process.cwd(), rel), "utf8");

  test("الحرجة ١ · تسديدُ دَينٍ يتجاوزه ⇒ `carry` سالبٌ (رصيدٌ للمشترك) لا صفر", () => {
    const src = read("src/app/api/debts/[id]/pay/route.ts");
    // 🔴 كان `Math.max(0, …)` يقصّ الزائد: مَن عليه ٥٠٬٠٠٠ ودفع ٦٠٬٠٠٠ يصير دَينُه صفراً
    //   **والعشرةُ آلافٍ تختفي** — دخلت الصندوقَ ولا أثرَ لها عند المشترك.
    assert.ok(/const newCarry = currentCarry - amount;/.test(src), "الطرحُ لم يبقَ صريحاً");
    assert.equal(/Math\.max\(0,\s*currentCarry - amount\)/.test(src), false,
      "أُعيد قصُّ الزائد عند صفر — فيختفي ما دفعه المشترك زائداً");
    assert.ok(/overpaid/.test(src), "الزائدُ لا يُذكَر في القيد فلا يُقرَأ لاحقاً");
  });

  test("الحرجة ٢ · تسديدُ مكتبٍ رصيدُه سالبٌ **يُقلَب صرفاً** لا يُرفض", () => {
    const src = read("src/app/api/money/settlements/route.ts");
    assert.ok(/الحرجة ٢/.test(src), "علّةُ المجموع السالب غيرُ معالَجةٍ في المسار");
    // القلبُ إلى صرفٍ: مبلغٌ سالبٌ يعني أنّ المكتبَ دائنٌ لا مدينٌ — فيُصرَف له
    assert.ok(/moneyOut/.test(src) && /moneyIn/.test(src), "لا طرفَي حركةٍ فلا قلبَ ممكن");
  });

  test("الحرجة ٣ · إرجاعُ قيدٍ لا يحذف تسديدةً بقيودٍ أخرى", () => {
    const src = read("src/app/api/money/settlements/route.ts");
    assert.ok(/الحرجة ٣/.test(src), "الإرجاعُ ما زال يحذف التسديدةَ كلَّها");
    assert.ok(/إن خلت/.test(src), "لا شرطَ «تُلغى إن خلت» — فقيدٌ واحدٌ يمحو تسديدةَ عشرة");
  });

  test("الحرجة ٤ · إبطالُ فاتورةٍ لا يمحو ما سدّده المشترك — والقصُّ **ظاهرٌ** لا صامت", () => {
    const src = read("src/app/api/invoices/[id]/void/route.ts");
    assert.ok(/الحرجة ٤/.test(src), "القصُّ ما زال صامتاً");
    assert.ok(/clampedDebt/.test(src), "لا متغيّرَ للمقصوص فلا يُسجَّل");
    // ويُكتَب في التدقيق بالأرقام: كان كذا ← طُرح كذا ← ولم يُطرح كذا
    assert.ok(/لم يُطرح/.test(src), "المقصوصُ لا يُكتَب في سجلّ التدقيق فلا يُراجَع");
  });

  test("الحرجة ٥ · واصلٌ أكبرُ من الفاتورة يُقيَّد **رصيداً** لا إيراداً", () => {
    const src = read("src/app/api/invoices/route.ts");
    assert.ok(/الحرجة ٥/.test(src), "الزائدُ ما زال يُقيَّد إيراداً");
    assert.ok(/رصيد/.test(src), "لا ذكرَ للرصيد — فالزائدُ يُبتلَع في الإيراد");
  });

  test("جذرُ الكسر · `/api/money` يرفض الكسرَ العشريَّ عند المدخل", () => {
    const src = read("src/app/api/money/route.ts");
    // 🔴 والعلّةُ المقيسة: سلفةٌ بكسرٍ عشريٍّ واحدٍ كانت **تُسقط تسديدَ الراتب كلَّه**
    //   (فيصير الصافي كسريّاً فلا يطابق أيَّ مبلغٍ صحيح).
    assert.ok(/\.int\(/.test(src), "المبلغُ يُقبَل كسريّاً — والعملةُ العراقيّةُ بلا كسور");
    assert.ok(/جذرُ الكسر/.test(src), "لا شرحَ للعلّة في المسار فتُعاد");
  });

  test("عالية · «ما سحبه» يطرح ما سُدِّد — فلا يبقى المبلغُ على الفنيّ أبداً", () => {
    const src = read("src/app/api/manager-accounts/route.ts");
    // قِيس: اسماعيل ٣٨٬٠٠٠ ظاهرةٌ عليه ومتبقّيه صفر — لأنّ الجمعَ كان بلا ترشيح
    assert.ok(/salaryStatementId: null/.test(src),
      "الجمعُ يشمل ما احتُسب في كشفٍ مُسدَّد — فيظهر المبلغُ على الفنيّ إلى الأبد");
  });

  // ⏳ والثامنةُ وحدَها لم تُبنَ: تسديدُ راتبٍ **صافيه سالب**. وهي تحتاج عمودَي
  //   `paidAmount`/`carryOut` على `SalaryStatement` لترحيلِ المتبقّي للشهر التالي —
  //   وقرارُ محمد فيها: «إذا أُعطي الموظّفُ راتبَه فلن يُمسَح شيءٌ له بعدها».
  test("تسديدُ راتبٍ سالبٍ يُدخل المال للصندوق: ١٠٠٬٠٠٠ − (−١٠٠) = ١٠٠٬١٠٠",
    { todo: "ب-٠٠ · يحتاج paidAmount/carryOut على SalaryStatement" }, () => {});
});
